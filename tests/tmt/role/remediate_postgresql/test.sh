...
rlJournalStart
    ...
    rlPhaseEnd
    rlPhaseStartTest "Role test: remediate postgresql"
        managed_node=$(echo "$managed_nodes" | awk '{print $1}')
        playbook="$coll_path/roles/remediate/tests/tests_leapp_old_postgresql_data.yml"
        LOGFILE="role-remediate_postgresql-${managed_node}-ANSIBLE-${SR_ANSIBLE_VER}"
