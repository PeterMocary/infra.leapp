...
rlJournalStart
    ...
    rlPhaseEnd
    rlPhaseStartTest "Role test: common default"
        managed_node=$(echo "$managed_nodes" | awk '{print $1}')
        playbook="$coll_path/roles/common/tests/tests_default.yml"
        LOGFILE="role-common_default-${managed_node}-ANSIBLE-${SR_ANSIBLE_VER}"
