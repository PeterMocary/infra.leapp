...
rlJournalStart
    ...
    rlPhaseEnd
    rlPhaseStartTest "Role test: remediate default"
        managed_node=$(echo "$managed_nodes" | awk '{print $1}')
        playbook="$coll_path/roles/remediate/tests/tests_default.yml"
        LOGFILE="role-remediate_default-${managed_node}-ANSIBLE-${SR_ANSIBLE_VER}"
