...
rlJournalStart
    ...
    rlPhaseEnd
    rlPhaseStartTest "Role test: analysis default"
        managed_node=$(echo "$managed_nodes" | awk '{print $1}')
        playbook="$coll_path/roles/analysis/tests/tests_default.yml"
        LOGFILE="role-analysis_default-${managed_node}-ANSIBLE-${SR_ANSIBLE_VER}"
